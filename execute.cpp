...
void setNegativeZero(int num, int bits) {
    // Check if Zero 
    if(bits == 8)
        num = signExtend8to32ui(num);
    if(bits == 11)
        num = signExtend11to32ui(num);
    if(bits == 16)
        num = signExtend16to32ui(num);
    ...
    case ALU:
      ...
      switch(add_ops) {
        ...
        case ALU_ADDR:
          ...
          setNegativeZero(rf[alu.instr.addr.rd], 32); /* Sets N,Z flag */
          setCarryOverflow(rf[alu.instr.addr.rn], rf[alu.instr.addr.rm], OF_ADD); /* Carry or overflow*/
          stats.numRegWrites++;
          stats.numRegReads += 2;
          break;
        case ALU_SUBR:
    ...
    case SPECIAL:
      ...
      switch(sp_ops) {
        case SP_MOV:
          ...
          setNegativeZero(rf[(sp.instr.mov.d << 3) | sp.instr.mov.rd], 32);
          stats.numRegReads++;
          stats.numRegWrites++;
          break;
        case SP_ADD:
          rf.write((sp.instr.add.d << 3) | sp.instr.add.rd, rf[(sp.instr.add.d << 3) | sp.instr.add.rd] + rf[sp.instr.add.rm]);
          setNegativeZero(rf[(sp.instr.add.d << 3) | sp.instr.add.rd], 32);
